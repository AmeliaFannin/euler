...
def lychrel
  ...
  10.upto(10000) do |num|
    @iterations = 0
    reverse_add(num)
    lychrel_nums += 1 if @iterations == 50
  end
...
def reverse_add(num)
  reverse_num = []
  num.to_s.each_char {|n| reverse_num << n}
  new_num = num + reverse_num.reverse.join.to_i
  @iterations += 1 

  reverse_add(new_num) if pal_check(new_num) && @iterations < 50
end

def pal_check(num)
  pal = []
  num.to_s.each_char {|n| pal << n }
  return true if num != pal.reverse.join.to_i
end
