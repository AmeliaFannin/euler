...
def sum(scope)
  ...
  puts "Collected #{abundants.length} abundant numbers"
  
  # collects sums of abundants into array
  x = abundants.length - 1
  non_matching_ints = []
  
  puts "Collecting Abundant Sums"
  ...
  # removes sums of abundants from integer array
  resulting_ints = Set.new(abundants) - Set.new(non_matching_ints)
